...
CREATE TABLE public.first_aid_info (
  ...
  price TEXT, duration TEXT, dates TEXT,
...
CREATE TABLE public.inquiries (
  ...
  contact_pref TEXT, message TEXT,
...
CREATE TABLE public.offers (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  image_url TEXT, headline TEXT, subline TEXT,
...
CREATE TABLE public.prices (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  old_price TEXT, offer_label TEXT,
  offer_active BOOLEAN NOT NULL DEFAULT false,
  offer_valid_from TIMESTAMPTZ, offer_valid_until TIMESTAMPTZ,
  offer_note TEXT
);
...
CREATE TRIGGER update_first_aid_info_updated_at BEFORE UPDATE ON public.first_aid_info FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
CREATE TRIGGER update_instagram_posts_updated_at BEFORE UPDATE ON public.instagram_posts FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
CREATE TRIGGER update_prices_updated_at BEFORE UPDATE ON public.prices FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
CREATE POLICY "Public read first aid" ON public.first_aid_info FOR SELECT TO anon, authenticated USING (active = true);
...
CREATE POLICY "Anyone submit inquiries" ON public.inquiries FOR INSERT TO anon, authenticated WITH CHECK (length(name) BETWEEN 1 AND 200 AND (email IS NULL OR length(email) <= 320) AND (phone IS NULL OR length(phone) <= 50) AND (message IS NULL OR length(message) <= 5000));
...
CREATE POLICY "Public read active instagram" ON public.instagram_posts FOR SELECT TO anon, authenticated USING (active = true);
CREATE POLICY "Admins manage instagram" ON public.instagram_posts FOR ALL TO authenticated USING (public.has_role(auth.uid(), 'admin')) WITH CHECK (public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Public read active prices" ON public.prices FOR SELECT TO anon, authenticated USING (active = true);
...
CREATE POLICY "Public read active team" ON public.team_members FOR SELECT TO anon, authenticated USING (active = true);
